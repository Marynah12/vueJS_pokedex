  ...
  <div class="detail">
    <div class="detail-view card">
      <div class="data card-body">
        <img
          class="image"
          :src="URLImage + '/' + detailPokemon.name + '.png'"
        />
        <h2 class="card-title">
          <span>{{ detailPokemon.name }}</span>
        </h2>
        <div class="types">
          <div
            ...
            class="type"
          >
            <span v-bind:class="type.type.name">{{ type.type.name }}</span>
          </div>
        </div>
        <div class="abilities">
          <div
            class="ability"
            v-for="ability in detailPokemon.abilities"
            v-bind:key="ability"
          >
        ...
        <div class="property">
          ...
          <div class="right">{{ detailPokemon.height }}</div>
        </div>
        <div class="property">
          <div class="left bold">Poids</div>
          <div class="right">{{ detailPokemon.weight }}</div>
        </div>
        <div class="property">
          <div v-bind:key="stat" v-for="stat in detailPokemon.stats">
            <li class="stats">
              {{ stat.stat.name }}
              :
              <div class="right">
...
export default {
  ...
  methods: {
    //  Fonction callback du bouton close
    closeDetail: function () {
      // Envoie d'un événement au parent : Pokedex
      this.$emit("pokemonClosed");
    },
  },
  //  Cycle de vie : avant que le composant soit monté
  beforeMount() {
    axios.get(this.pokemonURL).then((e) => {
      // Récupération des données du Pokémon correspondant à l'URL
      this.detailPokemon = e.data;
    });
  },
};
</script>
